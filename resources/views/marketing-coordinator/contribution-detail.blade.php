            ...
            <div class="contribution-view-wrap  contribution-form-wrap">
                ...
                <div class="contribution-btn-wrap">
                    <div class="btn-contribution">
                        <input type="hidden" name="id" value="{{ $contribution->academicYear_id }}">
                        <button class="cancel-btn" id="cancel-btn" data-route="{{ route('contributions') }}">Cancel</button>
                        @if($finalDeadline->greaterThan($currentDate))
                        @if($contribution->condition == 'approved')
                            @if($contribution->allowGuest == true)
                                <button class="un-allow-btn" id="un-allow-btn"  data-route="{{ route('unAllowGuest') }}">Un Allow Guest</button>
                            @else
                                <button class="allow-btn" id="allow-btn"  data-route="{{ route('allowGuest') }}">Allow Guest</button>
                            @endif
                        @endif
                        <button class="approved-btn" id="approved-btn"  data-route="{{ route('approved') }}">Approved</button>
                        <button class="rejected-btn" id="rejected-btn"  data-route="{{ route('rejected') }}">Rejected</button>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <section class="comment-contribution-wrapper" >
            <div class="comment-contribution-wrap" >
                ...
                <div class="commentBox-wrap">
                    <form action="{{ route('mc.comment')}}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('patch')
                        <input type="hidden" name="contribution_id" value="{{ $contribution->id }}">
                        <input type="hidden" name="student_id" value="{{ $contribution->user_id }}">
